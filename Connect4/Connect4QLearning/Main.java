...
//		try{
//			File f = new File("qtable.txt");
//			if (f.exists()){
//				System.out.println("Loading Q Table ...");
//				HashMap<String, String[]> qtable = new HashMap<>();
//				HashMap<String, int[]> action_count = new HashMap<>();
//				BufferedReader br = new BufferedReader(new FileReader(f));
//				String line;
//				while ((line = br.readLine()) != null){
//					line = line.substring(0, line.length()-1);
//					String[] spl = line.split(":");
//					qtable.put(spl[0], spl[1].split(" "));
//					String[] action_count_string = spl[2].split(" ");
//					int[] counts = new int[action_count_string.length];
//					for(int i=0;i<action_count_string.length;i++){
//						counts[i] = Integer.parseInt(action_count_string[i]);
//					}
//					action_count.put(spl[0], counts);
//				}
//				QLearnerAI.state_action_values = qtable;
//				QLearnerAI.state_action_count = action_count;
//			}else{
//				System.out.println("start training from scratch ...");
//			}
//		}catch (Exception e){
//			e.printStackTrace();
//		}
//	}
	...
	public static void main(String[] args)
	{
		...
		boolean loadQtable = false;
		boolean loadMCTS = false;
		...
		try
		{
			...
			while(i < args.length)
			{
				if(args[i].equalsIgnoreCase("-p1"))
					if (args[i + 1].equals("QLearnerAI")){
						players[0] = new QLearnerAI(0);
					}else
